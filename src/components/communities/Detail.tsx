...
export const CommunityDetail: React.FC = () => {
  ...
  const isOwner = community.members.find(m => m.id === user?.id)?.role === 'owner'
    ...
    <Stack spacing={6}>
      <Box bg={bgColor} borderRadius="lg" overflow="hidden" borderWidth="1px" borderColor={borderColor}>
        ...
        <Box p={6}>
          <Flex justify="space-between" align="flex-start">
            <Heading size="lg" mb={4}>{community.name}</Heading>
          ...
          <Text color="gray.500" mb={4}>
            {t('communities.member_count', { count: community.members.length })}
          </Text>
        </Box>
      </Box>
      ...
      <Tabs colorScheme="primary" isLazy>
        ...
        <TabPanels>
          ...
          <TabPanel px={0}>
            <Heading size="md" mb={4}>{t('communities.shared_tools')}</Heading>
            <ElementNotFound
              icon={icons.tools}
